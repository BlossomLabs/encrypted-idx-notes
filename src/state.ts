...
type NoteLoadedAction = { type: 'note loaded'; docID: string; doc: TileDocument, text: string }
...
export function useApp() {
  ...
  const saveDraft = useCallback(
    (title: string, text: string) => {
      ...
      ceramic!.did!.createJWE(fromString(text), [idx.id])
        .then(jwe => {
          console.log("Content strored in IDX:", jwe)
          return Promise.all([
            TileDocument.create(ceramic, jwe, {controllers: [idx.id], schema: schemas.EncryptedNote} ),
        ...
        .then(([doc, notesList]) => {
          ...
          return idx
            ...
            .then(() => {
              ...
              dispatch({ type: 'draft saved', docID, title, doc, text })
            })
        })
        .catch((err) => {
    ...
    (docID: string) => {
      ...
      if (state.notes[docID] == null || state.notes[docID].status === 'init') {
        ...
        TileDocument.load<Record<string, unknown>>(ceramic, docID).then(stream => {
          return ceramic!.did!.decryptJWE(stream.content).then(content => {
            const text = toString(content)
            dispatch({ type: 'note loaded', docID, doc: stream, text })
          })
        }).catch(()=> {
          dispatch({
            type: 'note loading status',
  ...
  const saveNote = useCallback(async (doc: TileDocument, text: string) => {
    const docID = doc.id.toString()
    const { ceramic, idx } = state.auth as AuthenticatedState
    dispatch({ type: 'note saving status', docID, status: 'saving' })
    try {
      await doc.update(await ceramic!.did!.createJWE(fromString(text), [idx.id]))
